...
| 9 | **Confirm the coverage tiers.** `coverage` lists four regions ordered by distance, with response expectations that differ between them. Towns are listed for Surrey/south-west London and for Kent & Sussex; Hertfordshire and the South West still name only Welwyn and Plymouth. The Kent & Sussex list is the principal towns of those counties, not a confirmed attendance list — strike any you do not actually serve, and add towns to the remaining regions as you confirm them. Named towns help local search, but an unserved town on the list is worse than a short list. | `src/data/site.mjs` |
